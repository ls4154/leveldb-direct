...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  void* buf;
  env_->OffloadBuffer(&buf);
  CompactionBuffer* cb = reinterpret_cast<CompactionBuffer*>(buf);
  versions_->MakeOffloadBuffer(cb, compact->compaction);
  cb->seq = compact->smallest_snapshot;

  int max_outputs = cb->out_cnt;
  std::vector<int> output_idxs;
  int offset = 2 * cb->in_cnt * sizeof(uint32_t) + 2 * cb->in2_cnt * sizeof(uint32_t);
  uint32_t* out_objs = (uint32_t*)&cb->data[offset];
  for (int i = 0; i < max_outputs; i++) {
    output_idxs.push_back(out_objs[i]);
  }

  // Release mutex while we're actually doing the compaction work
  mutex_.Unlock();

  env_->OffloadCompaction(buf, buf);
  ...
  fprintf(stderr, "offload task done\n");
  ResultBufffer* rb = (ResultBufffer*)buf;
  fprintf(stderr, "output cnt %d\n", rb->output_cnt);

  mutex_.Lock();
  fprintf(stderr, "apply output file infos\n");
  uint32_t* out_sizes = (uint32_t*)&rb->data[0];
  offset = rb->output_cnt * sizeof(uint32_t);
  for (int i = 0; i < rb->output_cnt; i++) {
    CompactionState::Output out;
    ...
    out.largest.DecodeFrom(Slice(&rb->data[offset], klen));
    fprintf(stderr, " largest %s\n", out.largest.user_key().ToString().c_str());
    offset += klen;
